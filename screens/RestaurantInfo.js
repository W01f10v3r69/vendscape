...
const RestaurantInfo = ({ navigation, route }) => {
	const { banner, name, description, vegan, price, vendorsId, FoodId } = route.params;
	...
	const postReview = (rant) => {
		const url = `https://floating-wildwood-95983.herokuapp.com/review/`;
		axios
			.post(url, { rant, reviewerId: ownersid, FoodId: FoodId._id }, {
				hedaers: {
	...
	const getReview = () => {
		const url = `https://floating-wildwood-95983.herokuapp.com/review/getReview/${FoodId._id}`;
		axios
			.get(url)
			.then((response) => {
				const result = response.data;
		...
		<View style={tw`flex relative bg-white`}>
			...
			<View style={tw.style("flex bg-white h-full")}>
				...
				>
					...
					<View style={[tw.style("mt-10 w-4/5", {}), { width: "100%" }]}>
						<View style={tw`w-full flex flex-col`}>
							<BoltSemiBoldText
								style={tw.style("text-lg", {
									flexWrap: "wrap",
							...
							{
								review.map((items)=>{
									return	<Review pro_pic={items.pic} rant={items.rant} reviewer={items.name} />
								})
							}
							...
							<AppTextInput
								...
								value={newR}
							/>
							<AppButton title="SUBMIT REVIEW" onPress={() => postReview(newR)} />
							<View style={{ height: 100, width: "100%", backgroundColor: "transparent" }}></View>
						</View>
					</View>
